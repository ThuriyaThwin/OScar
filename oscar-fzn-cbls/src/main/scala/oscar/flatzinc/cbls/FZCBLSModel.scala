...
class FZCBLSModel(val fzModel: FZProblem, val log: Log, val getWatch: () => Long, val opts: Options) {
  
  
  // Model
  val m: Store = Store(verbose = false,
                        checker = None) //setting the last Boolean to true would avoid calling the SCC algorithm but we have to make sure that there are no SCCs in the Graph. Is it the case in the way we build it?
  // constraint system
  val c = ConstraintSystem(m)
  
  val cblsIntMap: MMap[String, IntValue] = MMap.empty[String, IntValue]
  
  // IteratorVariables in a declarative neighbourhood can be accidentally defined by a constraint, unset them before creating all variables (as defined variables are skipped).
  fzModel.neighbourhoods.foreach(_.subNeighbourhoods.foreach(_.itVariables.foreach(v => if(v.isDefined) v.definingConstraint.get.unsetDefinedVar(v))))
  ...
  def addDefaultNeighbourhoods(){
    if (vars.length > 0) {
      ...
      /*
      val varsToSwap = vars.groupBy(_.domainSize)
      println(varsToSwap.mkString("\n"))
      println(varsToSwap.mapValues(_.length).mkString("\n"))

      val splitVars = vars.groupBy( v => v.domainSize == 2 || v.name == "objectiveFun")
*/
      //TODO: take care of this split!
      val splitVars = fzModel.variables.filter(v => v.isSearchVar && vars.exists(_.name == v.id))
      val searchVars = splitVars.map( v => cblsIntMap(v.id).asInstanceOf[CBLSIntVar]).toArray
      if(searchVars.size > 0)
        addNeighbourhood((o,c) => new MaxViolating(searchVars, c), searchVars)


      class GenericMaxViolating(searchVariables: Array[CBLSIntVar],
                                constraintSystem: ConstraintSystem)
        extends MaxViolating(searchVariables,constraintSystem) with GenericNeighbourhood

      if(vars.length > 0)
        addNeighbourhood((o,c) => new GenericMaxViolating(vars.toArray, c),Array.empty[CBLSIntVar])


      //addNeighbourhood((o,c) => new GenericMaxViolating(splitVars(true).toArray, o, c),Array.empty[CBLSIntVar])

      //addNeighbourhood((o,c) => new MaxViolating(splitVars(false).toArray, o, c),Array.empty[CBLSIntVar])
  ...
  def createNeighbourhoods() {
    ...
    neighbourhoods.foreach(n => {
      log(2,
           "Created Neighbourhood " + n + " over " + n.searchVariables.length + " variables")
    })
  }
    ...
    for (parsedVariable <- fzModel.variables if !parsedVariable.isDefined) {
      parsedVariable match {
        case IntegerVariable(id, dom, _) =>
          ...
          val initialValue = dom match {
            case oscar.flatzinc.model.FzDomainRange(min, max) =>
              if (max.toLong - min.toLong > Int.MaxValue) {
                Random.nextInt()
              } else {
          ...
          val sc = if (parsedVariable.cstrs.nonEmpty) parsedVariable.cstrs.map {
            case c: subcircuit => c.variables.length;
            case c: circuit => c.variables.length;
            case _ => 0
          }.min else 0//Min here, right?
        ...
        case bv: BooleanVariable =>
          ...
          val dom = DomainRange(if (bv.isTrue) 0 else if (bv.isFalse) 1 else 0,
                                 if (bv.isFalse) 1 else if (bv.isTrue) 0 else 1)
          //TODO: Put this in a method! or make it deterministic as the neighbourhoods should take care of the assignments!
          val initialValue = {
    ...
    v match {
      case v: IntegerVariable =>
        cblsIntMap.get(v.id) match {
          ...
          case None => throw new RuntimeException("could not find variable " + v.id)
        }
  ...
  def getOutputOfCurrentAssignment: String = {
    fzModel.solution.getSolution {
      s: String => {
        cblsIntMap.get(s) match {
          case Some(intVar) =>
